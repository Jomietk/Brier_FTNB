...
class MDLP_Discretizer(object):
    ...
    def __init__(self, dataset, testset, class_label, out_path_data, out_test_path_data, out_path_bins, features=None, min_bins=None, min_freq=None):
        ...
        # class needs a pandas dataframe
        if not isinstance(dataset, pd.core.frame.DataFrame):
            raise AttributeError('Input dataset should be a pandas data frame')

        # class needs a pandas dataframe
        if not isinstance(testset, pd.core.frame.DataFrame):
        ...
        if features:
            ...
            # specified columns not in dataframe
            missing = set(features) - set(self._features)
            if missing:
                print(
                    'WARNING: user-specified features %s not in input dataframe' % str(missing))
        ...
        #print(self._features)                                                                                here if like before
        self.min_dict = {}
        self.max_dict = {}
        for i in self._features:
            range_min, range_max = self._data_raw[i].min(
            ), self._data_raw[i].max()
        ...
        self._ignored_features = set(
            self._data_raw.columns) - set(self._features)
        self._ignored_features_t = set(
            self._test_raw.columns) - set(self._features)
        ...
        self.apply_cutpoints(out_data_path=out_path_data,
                             out_test_path=out_test_path_data, out_bins_path=out_path_bins)
    ...
    def feature_boundary_points(self, data, feature):
        ...
        data_partition['feature_offset'] = data_partition[feature].shift(
            1)  # column where first value is now second, and so forth
        data_partition['feature_change'] = (
            data_partition[feature] != data_partition['feature_offset'])
        data_partition['mid_points'] = data_partition.loc[:,
                                                          [feature, 'feature_offset']].mean(axis=1)
        ...
        for row in potential_cuts:
            old_value = data_partition.loc[sorted_index[sorted_index.index(
                row) - 1]][feature]
            new_value = data_partition.loc[row][feature]
            old_classes = data_partition[data_partition[feature]
                                         == old_value][self._class_name].unique()
            new_classes = data_partition[data_partition[feature]
                                         == new_value][self._class_name].unique()
    ...
    def best_cut_point_min_freq(self, data, feature):
        ...
        if not candidates:
            return None
        gains = [(cut, cut_point_information_gain(dataset=data, cut_point=cut, feature_label=feature, class_label=self._class_name), [sum(i[0] for i in self.frequency_partition(
            data=data, feature=feature, cut_point=cut)), sum(i[1] for i in self.frequency_partition(data=data, feature=feature, cut_point=cut))]) for cut in candidates]
        gains = sorted(gains, key=lambda x: x[1], reverse=True)
    ...
    def single_feature_accepted_cutpoints(self, feature, partition_index=pd.DataFrame().index):
        ...
        if partition_index.size == 0:
            # if not specified, full sample to be considered for partition
        ...
        if not decision:
            ...
            if feature in self._partition_dict:
                ...
                if cut_candidate != None:
                    ...
                    self._partition_freq[feature] = [self.frequency_partition(
                        data=data_partition, feature=feature, cut_point=cut_candidate)]
        ...
        for i in range(bins):
            ...
            for partition_index, cut_candidate, partition_freq in zip(all_data_partitions, all_data_candidates, all_data_frequency):
                ...
                if len(data_partition[feature].unique()) < 2:
                    ...
                    gain_list.append(cut_point_information_gain(
                        dataset=data_partition, cut_point=cut_candidate, feature_label=feature, class_label=self._class_name))
            ...
            if max(gain_list) != 0:
                ...
                data_partitions_chosen = self._data.loc[data_partitions_chosen, [
                    feature, self._class_name]]

                # now we have two new partitions that need to be examined
                left_partition = data_partitions_chosen[data_partitions_chosen[feature]
                                                        <= cut_candidate_chosen]
                left_cut_candidate = self.best_cut_point_min_freq(
                    data=left_partition, feature=feature)
                right_partition = data_partitions_chosen[data_partitions_chosen[feature]
                                                         > cut_candidate_chosen]
                right_cut_candidate = self.best_cut_point_min_freq(
                    data=right_partition, feature=feature)
                ...
                if left_cut_candidate != None:
                    ...
                    all_data_frequency.append(self.frequency_partition(
                        data=left_partition, feature=feature, cut_point=left_cut_candidate))
                if right_cut_candidate != None:
                    ...
                    all_data_frequency.append(self.frequency_partition(
                        data=right_partition, feature=feature, cut_point=right_cut_candidate))
    ...
    def all_features_min_criteria_cutpoints(self):
        ...
        for attr in self._features:
            if len(self._cuts[attr])+1 < self._min_bins:
                ...
                self.single_features_min_criteria_cutpoints(
                    feature=attr, bins=bins_required)
        return
    ...
    def frequency_partition(self, data, feature, cut_point):
        ...
        for c in classes:
            ...
            freq_list.append([len(data_classes_l[data_classes_l == c]), len(
                data_classes_r[data_classes_r == c])])
        return freq_list
    ...
    def ranges(self):
        ...
        for key, value in self._cuts.items():
            if len(self._cuts[key]) != 0:
                ...
                ranges_dict[key] = [self.min_dict[key]] + \
                    value + [self.max_dict[key]]
        return ranges_dict
    ...
    def apply_cutpoints(self, out_data_path=None, out_test_path=None, out_bins_path=None):
        ...
        for attr in self._features:
            ...
            if len(self._cuts[attr]) == 0:
                ...
                self._data[attr] = 1
                self._test[attr] = 1
                self._frequencies[attr] = [[len(self._data.loc[(self._data[attr] == 1) & (
                    self._data[self._class_name] == j)])] for j in self._classes if len(self._cuts[attr]) == 0]
                self._frequencies_test[attr] = [[len(self._test.loc[(self._test[attr] == 1) & (
                    self._test[self._class_name] == j)])] for j in self._classes if len(self._cuts[attr]) == 0]
                pbin_label_collection[attr] = ['No binning']
                bin_label_collection[attr] = ['All']
            else:
                cuts = [-np.inf] + self._cuts[attr] + [np.inf]
                #print(attr, cuts)                                                hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
                start_bin_indices = range(0, len(cuts) - 1)
                pbin_labels = ['%s_to_%s' % (str(cuts[i]), str(
                    cuts[i+1])) for i in start_bin_indices]
                ...
                self._test[attr] = pd.cut(x=self._test[attr].values, bins=cuts, right=False, labels=bin_labels,
                                          precision=6, include_lowest=True)
                self._frequencies[attr] = [[len(self._data.loc[(self._data[attr] == i) & (
                    self._data[self._class_name] == j)]) for i in bin_labels] for j in self._classes if len(self._cuts[attr]) != 0]
                self._frequencies_test[attr] = [[len(self._test.loc[(self._test[attr] == i) & (
                    self._test[self._class_name] == j)]) for i in bin_labels] for j in self._classes if len(self._cuts[attr]) != 0]
        ...
        if self._ignored_features:
            ...
            to_return_train = pd.concat(
                [self._data, self._data_raw[list(self._ignored_features)]], axis=1)
            # sort columns so they have the original order
            to_return_train = to_return_train[self._data_raw.columns]
        else:
            # the line below may help in removing double class column ; looks like it works
            self._data = self._data.loc[:, self._features]
        ...
        if self._ignored_features:
            ...
            to_return_test = pd.concat(
                [self._test, self._test_raw[list(self._ignored_features_t)]], axis=1)
            # sort columns so they have the original order
            to_return_test = to_return_test[self._test_raw.columns]
        else:
            # the line below may help in removing double class column ; looks like it works
            self._test = self._test.loc[:, self._features]
        ...
        if out_bins_path:
            ...
            for i in self._features:
                ...
                range_list.append([self.min_dict[i]] +
                                  self._cuts[i] + [self.max_dict[i]])
                if len(self._frequencies[i]) != 0:
                    frequencie_list_train.append(self._frequencies[i])
                else:
                    frequencie_list_train.append(
                        ['Binning not possible']*len(self._classes))
                if len(self._frequencies_test[i]) != 0:
                    frequencie_list_test.append(self._frequencies_test[i])
                else:
                    frequencie_list_test.append(
                        ['Binning not possible']*len(self._classes))

            df_dis = pd.DataFrame(
                data={"Feature": feature_list, "Range": range_list})
            frequencie_list_train = np.array(
                frequencie_list_train, dtype=object)
            
            frequencie_list_test = np.array(frequencie_list_test, dtype=object)
            if len(self._features) > 1:
                for i in range(len(self._classes)):
                    df_dis['Frequency_train [ ' +
                           str(self._classes[i])+' ]'] = frequencie_list_train[:, i]
                    df_dis['Frequency_fwd [ ' +
                           str(self._classes[i])+' ]'] = frequencie_list_test[:, i]
            else:
                for i in range(len(self._classes)):
                    df_dis['Frequency_train [ '+str(self._classes[i])+' ]'] = str(
                        list(frequencie_list_train[0][i]))
                    df_dis['Frequency_fwd [ '+str(self._classes[i])+' ]'] = str(
                        list(frequencie_list_test[0][i]))
